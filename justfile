...
# Check components (workspace unsupported at this time)
check:
    $(cd verifier && cargo component c -r)
    RISC0_DEV_MODE=1 cargo c

# Build all components (workspace unsupported at this time)
build:
    # Build components
    $(cd verifier && cargo component b -r)

    # Build & generate proof file
    RISC0_DEV_MODE=0 cargo r --bin host
...
# Transpile for web
js: build
    jco transpile target/wasm32-wasi/release/verifier.wasm -o js/vite/bindings
